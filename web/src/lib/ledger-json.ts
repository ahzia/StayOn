...
export function upsertPostbackJson(
...
export function getPendingForUserJson(userId: string): LedgerEntry[] {
  const store = ensureStore();
...
export function markSyncedJson(userId: string, transIds: string[]): number {
  const store = ensureStore();
  let count = 0;
...
export function getUserSummaryJson(userId: string): {
...
export function readLedgerStore(): LedgerStore {
  return ensureStore();
}
